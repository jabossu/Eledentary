...
else
{
	...
	{	
		...
		{	
			$article = new article($donnee) ;
			if ( $mode == 'create' )
			{
				$am->add($article) ;
                log_add($_SESSION['profile']->id(), 'New article posted', 'Wrote an article named <i>' . $article->titre('en_EN') . '</i>' ) ;
				unset($_POST) ;
			}
			elseif ( $mode == 'edit' )
			{
				$am->update($article) ;
                log_add($_SESSION['profile']->id(), 'Post edited', 'Edition of the post #' . $_GET['id'] . ' named <i>' . $article->titre('en_EN') . '</i>' ) ;
				$errorType = 0 ;
			}
		}
	}
