...
$am = new dentissimoManager($bdd) ;

clean($_GET) ;
if (	isset( $_GET['action'] ) AND $_GET['action'] == 'edit' AND
		isset( $_GET['id'] ) AND $am->existeId( $_GET['id'] ) )
{	$mode = 'edit' ;
	$donnee['id'] 			= $_GET['id'] ;
}
...
// On charge l'dentissimo à éditer pour l'afficher dans le formulaire.
if ( $mode == 'edit' )
{
	if ( contains_null($_POST) or missing_keys($_POST, $parametresAttendus) )
	{
		$tmpNews = $am->get($_GET['id']) ;
...
else
{
	...
	{	
		...
		{	
			$dentissimo = new dentissimo($donnee) ;
			if ( $mode == 'create' )
			{
				$am->add($dentissimo) ;
                log_add($_SESSION['profile']->id(), 'New dentissimo posted', 'Wrote an dentissimo named <i>' . $dentissimo->titre('en_EN') . '</i>' ) ;
				unset($_POST) ;
			}
			elseif ( $mode == 'edit' )
			{
				$am->update($dentissimo) ;
                log_add($_SESSION['profile']->id(), 'Post edited', 'Edition of the post #' . $_GET['id'] . ' named <i>' . $dentissimo->titre('en_EN') . '</i>' ) ;
				$errorType = 0 ;
			}
		}
	}
